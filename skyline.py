...
## Donat els vertexs esencials et retorna els vertexs i codes pel Path -------MAYBE NOT NECESSARY
            ...
            if(i in dict1 and i in dict2):
                ...
                if((dict1[i] != y and dict2[i] != y)):
                    if(dict1[i] <= dict2[i]): 
                        ...
                        y = dict2[i]
                        left = i
    if(left != -1 and right != -1): valids.append((left,y,right))
    return valids 
...
def get_plot(buildings):
    ...
    #patch = patches.Patch(poly, facecolor='green', lw=2)
    ax.add_patch(poly)
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    ax.yaxis.set_major_locator(MaxNLocator(integer=True))
    #ax.set_xlim(0,weight)
